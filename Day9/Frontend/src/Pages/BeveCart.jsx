...
import getBeveById from './getBeveById'
...
const BeveCart = () => {
  const { id } = useParams();
  const BeveId = parseInt(id, 10);
  const BeveDetails = getBeveById(BeveId);

  if (!BeveDetails) {
    return <div>Beve not found</div>;
  }
  ...
  useEffect(() => {
    ...
    if (BeveDetails.images && BeveDetails.images.length > 0) {
      setImages(BeveDetails.images);
    }
  }, [BeveDetails]);
      ...
      <Container fluid>
        <Row>
          <Col className='cart-header'>
            ...
            <a href='/descBeve' className='header-bold text-decoration-none'>Dairy Poultry</a>
            <a href='#' className='header-bold text-decoration-none'>Oils Foodgrains</a>
            <a href='#' className='header-bold text-decoration-none'>Beverages</a>
            <a href='#' className='header-bold text-decoration-none'>Snacks</a>
            <a href='#' className='header-bold text-decoration-none'>Meat Fish</a>
            <a href='#' className='header-bold text-decoration-none'>Bakery Cakes</a>
          </Col>
        </Row>
        ...
        <Row className='cart-maincontainer'>
          <Col className='cart1-enclosure d-flex '>
            ...
            <Row className='cart-content'>
              <Col className='ml-1'>
                <p className='fresho1 fw-light'></p>
                <p className='name'>{BeveDetails.name}</p>
                <p className='ratings'>
                ...
                <p className='price'>Price: Rs.{BeveDetails.price}</p>
            ...
            <Col >
            <Button className='btncart1' onClick={()=>send(BeveDetails)}>Add to Basket <ShoppingBagIcon style={{ fontSize: '20px' }} /></Button>
            </Col>
