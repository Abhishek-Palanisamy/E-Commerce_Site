...
import getAmulById from './getAmulById'
...
const AmulCart = () => {
  const { id } = useParams();
  const AmulId = parseInt(id, 10);
  const AmulDetails = getAmulById(AmulId);

  if (!AmulDetails) {
    return <div>Amul not found</div>;
  }
  ...
  useEffect(() => {
    ...
    if (AmulDetails.images && AmulDetails.images.length > 0) {
      setImages(AmulDetails.images);
    }
  }, [AmulDetails]);
          ...
          <Col className='cart1-enclosure d-flex '>
            ...
            <Row className='cart-content'>
              <Col className='ml-1'>
                ...
                <p className='name'>{AmulDetails.name}</p>
                <p className='ratings'>
                ...
                <p className='price'>Price: Rs.{AmulDetails.price}</p>
            ...
            <Col >
            <Button className='btncart1' onClick={()=>send(AmulDetails)}>Add to Basket <ShoppingBagIcon style={{ fontSize: '20px' }} /></Button>
            </Col>
